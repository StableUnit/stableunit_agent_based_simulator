...
const countries = ['Russia', 'United States', 'Germany', 'UK', 'Japan', 'South Korea', 'Venezuela', 'Canada'];
...
function cryptoCompanyGetsHacked(): string {
  return headline([random(cryptoCompanies), 'gets hacked. Millions of accounts leaked']);
}
...
function newBullRun(): string {
  return headline([random(majorCryptos), 'has doubled over the past month. The bull trend continues']);
}
...
function countryLegalize(): string {
  return headline([random(countries), 'to legalize', random(cryptoProjectTypes)]);
}

function walletsCompromised(): string {
  return headline(['The most popular', random(majorCryptos), 'wallet is compromised. Millions of dollars are stolen']);
}

function warDeclaration(): string {
  return headline([random(countries), 'has declared war to', random(countries) + '.', random(majorCryptos), 'on local markets are skyrocketing']);
}

function valleyInvestsInCrypto(): string {
  return headline([random(valleyGiants), 'is launching an ICO to develop their own blockchain']);
}

function valleyBan(): string {
  return headline([random(valleyGiants), 'is blocking everything related to crypto']);
}
...
function countryElection(): string {
  return headline([random(countries) + 'ʼs', 'next election is going to be on blockchain']);
}

function badRidiculous(): string {
  return headline([random(['Satoshi Nakamoto is found smoking crack on streets of Vancouver'])]);
}

function positiveSpecialists(): string {
  return headline(['Technical analysis predicts the new highs for', random(majorCryptos)]);
}
...
const badNews: NewsGenerators = [cryptoCompanyDDoS, countryBansProject, valleyBan, walletsCompromised, negativeSpecialists, cryptoCompanyGetsHacked];
const goodNews: NewsGenerators = [valleyInvestsInCrypto, countryLegalize, positiveSpecialists, countryElection];
const badBlackSwans: NewsGenerators = [warDeclaration, cryptoCompanyGoesBankrupt, majorCryptoCrash, badRidiculous];
const goodBlackSwans: NewsGenerators = [majorCryptoAllTimeHighs, newBullRun, leadersHodling];
