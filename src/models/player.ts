...
type CallbackFunction = (arg0: {}) => void;
export type MapPlayerToProps<T> = (arg0: T) => {};
type EventPayload<T> = {
...
class Player {
  ...
  intervalId: NodeJS.Timeout | null | undefined = null;
  ...
  placeLimitBuyOrder = (order: BuySellOrder) => {
    this.status = this.simulation.market_SUETH.newLimitBuyOrder(order.trader, Number(order.quantity), Number(order.price) * Number(order.quantity));
  };
  placeLimitSellOrder = (order: BuySellOrder) => {
    this.status = this.simulation.market_SUETH.newLimitSellOrder(order.trader, Number(order.quantity), Number(order.price) * Number(order.quantity));
  };
  placeMarketBuyOrder = (order: BuySellOrder) => {
    this.status = this.simulation.market_SUETH.buyMarketOrder(order.trader, Number(order.quantity));
  };
  placeMarketSellOrder = (order: BuySellOrder) => {
    this.status = this.simulation.market_SUETH.sellMarketOrder(order.trader, Number(order.quantity));
  };
