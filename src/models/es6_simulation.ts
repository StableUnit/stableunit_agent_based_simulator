...
class Ethereum {
  ...
  createWallet(initial_amount: number = 0) {
    ...
    const public_key = Array(40).fill(0).map(() => "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random() * 62))).join("");
...
export class StableUnit extends Ethereum {
  ...
  sendTransaction(address_sender: string, amount: number, address_recipient: string) {
    ...
    if ((this.accounts.get(address_sender) || 0) * (1.0 - this.PARKING_ratio) >= amount) {
      return super.sendTransaction(address_sender, amount, address_recipient);
    }
    return false;
  }
...
export class Market_SUmETH extends Market {
  ...
  getCurrentBuyPrice() {
    if (this.buy_orders.length > 0) return this.buy_orders.slice(-1)[0].price;else return NaN;
  }

  getCurrentSellPrice() {
    if (this.sell_orders.length > 0) return this.sell_orders.slice(-1)[0].price;else return NaN;
  }
...
export const market_SUETH = new Market_SUmETH(2
/* mETH per SU */
);
