...
test('market to initialize', () => {
  expect(simulation.market_SUETH).toBeDefined(); //expect(simulation.traders.size).toBe(7);
});
test("market works correct", () => {
  ...
  const trader_1 = new Trader("trader_1", {
    balance_SU: 1000,
    balance_mETH: 2000
  });
  const trader_2 = new Trader("trader_2", {
    balance_SU: 1000,
    balance_mETH: 2000
  });
  ...
  const trader_3 = new Trader("trader_3", {
    balance_SU: 0,
    balance_mETH: 10
  });
  ...
  const trader_4 = new Trader("trader_4", {
    balance_SU: 2000,
    balance_mETH: 1001
  });
